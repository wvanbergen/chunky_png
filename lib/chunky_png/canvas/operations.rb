module ChunkyPNG
  class Canvas
    ...
    module Operations
      ...
      # Composes another image within bounds onto this image using alpha blending. This will modify
      # the current canvas.
      #
      # If you simply want to replace pixels or when the other image does not have
      # transparency, it is faster to use {#replace!}.
      ...
      #     given the offset and size of the other canvas.
      # @see #compose!
      def compose_within_bounds!(other, offset_x = 0, offset_y = 0)
        for y in 0...other.height do
          for x in 0...other.width do
            set_pixel_if_within_bounds(x + offset_x, y + offset_y, ChunkyPNG::Color.compose(other.get_pixel(x, y), get_pixel(x + offset_x, y + offset_y)))
          end
        end
        self
      end
